...
public class GUI_ParkirMobil extends javax.swing.JFrame {
 ...
 try {
            ...
            // Jika memilih Kendaraan Standart (Index 0)
            if (cmbPilihKendaraan.getSelectedIndex() == 0) { 
                k = new ParkirMobil(nama, plat, merk);
                infoJenis = "Kendaraan Standart";
                
                // KENDARAAN STANDART KENA DENDA KARENA PARKIR DI ZONA LISTRIK
                int denda = 50000; 
                totalBiaya = k.hitungBiaya(lama, denda); // Memanggil Method Overloading
                statusDenda = "Rp " + denda;
                
                JOptionPane.showMessageDialog(this, "PERINGATAN: Kendaraan standart terkena denda karena parkir di zona khusus mobil listrik!", "Pelanggaran Zona", JOptionPane.WARNING_MESSAGE);
            
            // Jika memilih Kendaraan Listrik (Index 1)
            } else { 
                k = new MobilElektrik(nama, plat, merk);
                infoJenis = "Kendaraan Listrik";
                
                // Kendaraan listrik aman, pakai tarif normal
                totalBiaya = k.hitungBiaya(lama); // Memanggil Method Biasa (Override)
                statusDenda = "-";
            }

            // Memasukkan data ke JTable dengan Kolom yang terpisah (7 Kolom)
            ...
            batal(); // Bersihkan form otomatis setelah simpan

        } catch (NumberFormatException e) {
            JOptionPane.showMessageDialog(this, "Pastikan Lama Parkir diisi dengan angka!", "Input Error", JOptionPane.ERROR_MESSAGE);
